...
function encryption() {
    ...
    document.querySelector("#encrypt-btn").addEventListener("click", function () {
        ...
        // saving emojis and data in localStorage
        var dataarr = [];
...
function decryption() {
    document.querySelector("#decrypt-btn").addEventListener("click", function () {
        ...
        var found;
        for (let i of user) {
            // Check if both the encrypted string (clutter) and password match
            if (i.clutter.trim() == clutter2.trim() && i.pass === finalPass) {
                found = i;
                console.log(i);
            }
        }

        // If both the clutter and password match, show the decrypted message
        if (found) {
            document.querySelector("#result").style.display = `block`;
            document.querySelector("#result").style.color = `#eee`;
            document.querySelector("#result").innerHTML = found.input;
        } else {
            // If password is incorrect or encrypted message doesn't match, show error
            document.querySelector("#result").style.display = `block`;
            document.querySelector("#result").style.color = `red`;
            document.querySelector("#result").innerHTML = "Enter correct password!";
        }
    });
}
